...
function lookInModuleManifest(req, service) {
  try {
    ...
    if (servicePackage.luxNode) {
      serviceModule = serviceModule + '/' + servicePackage.luxNode;
...
function loadModule(req, service) {
  ...
  //package.json file, if so, then see if there is a luxNode field, if so
  ...
  if (!serviceCode) {
    throw new Error('Attempted to load the ' + service.name + ' service from: ' +
      ...
      process.cwd() + '/package.json" - luxNode field. All paths failed to find valid nodeJS code.');
  }
