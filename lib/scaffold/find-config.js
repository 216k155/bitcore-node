...
/**
 * Will return the path and lux-node configuration
...
function findConfig(cwd) {
  ...
  while (!fs.existsSync(path.resolve(directory, 'lux-node.json'))) {
  ...
  return {
    path: directory,
    config: require(path.resolve(directory, 'lux-node.json'))
  };
}
