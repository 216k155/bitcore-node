...
var BASE_PACKAGE = {
  description: 'A full Bitcoin node build with lux',
...
/**
 * Will create a base lux Node configuration directory and files.
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/lux-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a lux Node directory, configuration file,
 * bitcoin configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the lux node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the lux-node directory and configuration
