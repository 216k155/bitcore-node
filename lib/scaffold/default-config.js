...
/**
 * Will return the path and default lux-node configuration. It will search for the
 * configuration file in the "~/.lux" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.lux');
  var defaultConfigFile = path.resolve(defaultPath, './lux-node.json');
  ...
  if (fs.existsSync(defaultConfigFile)) {
    ...
    console.log(`The configuration file at '${defaultConfigFile}' is incompatible with this version of lux.`);

    var now = new Date();
    // lux-node.YYYY-MM-DD.UnixTimestamp.json
    var backupFileName = `lux-node.${now.getUTCFullYear()}-${now.getUTCMonth()}-${now.getUTCDate()}.${now.getTime()}.json`;
