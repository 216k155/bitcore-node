...
var $ = lux.util.preconditions;
var _ = lux.deps._;
var utils = require('../utils');

/**
 * Will remove a service from lux-node.json
...
/**
 * Will remove the Node.js service and from the lux-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The lux-node configuration directory
...
function remove(options, done) {
  ...
  var luxConfigPath = path.resolve(configPath, 'lux-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(luxConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a lux-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to lux-node.json
        removeConfig(luxConfigPath, service, next);
      });
    }, done
  );
}
